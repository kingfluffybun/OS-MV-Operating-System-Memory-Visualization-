...
function showMenu() {
  ...
  const isComparisonPage = currentPath.includes('/simulator/compare/index.html') || currentPath.includes('/simulator/compare/simulation/');
...
function isSegmentationPagingMode() {
  ...
  if (sessionStorage.getItem('selectedAlgo')?.toLowerCase() === 'seg-paging') return true;
...
const runStep = () => {
  ...
  if (isSegmentationPaging) {
    ...
    if (simulationState.subStep === 0) {
      ...
      console.log(`Performing Physical Allocation for ${processName} (Page ${simulationState.pageAllocationIndex})`);
      ...
      if (currentProcess && currentProcess.segments) {
        ...
        if (simulationState.pageAllocationIndex < allPages.length) {
          const { segmentType, page } = allPages[simulationState.pageAllocationIndex];
          const allocationResult = PagingSegmentSimulator.allocatePageStepSingle(
        ...
        if (simulationState.pageAllocationIndex >= allPages.length) {
          appendConsoleMessage(`${processName} -> All pages mapped to physical frames.`);
          ...
          simulationState.stats.intFragmentation += currentProcess.internalFragmentation;
...
const getActiveView = () => {
  ...
  let activeView = null;
  if (standardView && standardView.style.display !== 'none') activeView = standardView;
  else if (pagingView && pagingView.style.display !== 'none') activeView = pagingView;
  else if (segmentationView && segmentationView.style.display !== 'none') activeView = segmentationView;
  else if (segPagingView && segPagingView.style.display !== 'none') activeView = segPagingView;
