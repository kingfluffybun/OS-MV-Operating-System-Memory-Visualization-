...
function isPagingMode() {
  ...
  return (
    isPaging ||
    (selectedAlgo && selectedAlgo.toLowerCase() === "paging") ||
    (urlAlgo && urlAlgo.toLowerCase() === "paging")
  );
}
...
function isSegmentationMode() {
  ...
  return (
    isSegmentation ||
    (selectedAlgo && selectedAlgo.toLowerCase() === "segmentation") ||
    (urlAlgo && urlAlgo.toLowerCase() === "segmentation")
  );
}
...
function isSegmentationPagingMode() {
  ...
  if (segPagingView.style.display === "grid") return true;

  const urlParams = new URLSearchParams(window.location.search);
  if (urlParams.get("algorithm")?.toLowerCase() === "seg-paging") return true;
  if (sessionStorage.getItem("selectedAlgo")?.toLowerCase() === "seg-paging")
    return true;

  return !document.getElementById("paging-view");
}
...
function attachProcessListeners() {
  ...
  const mainGrid =
    ...
    document.querySelector(".main-grid.segmentation");
  ...
  if (addProcessBtn) {
    ...
    newAddBtn.addEventListener("click", () => {
      ...
      if (isSegmentationPagingMode()) {
        ...
        const nextId =
          segPagingContainer.querySelectorAll(".process").length + 1;
        ...
        if (sizeInput) sizeInput.value = "";
        segContainer.scrollTo({
          top: segContainer.scrollHeight,
          behavior: "smooth",
        });
      } else {
...
const prepareSimulation = () => {
  ...
  if (isSegmentationPaging) {
    ...
    // Calculate total pages needed for all processes (4 segments per process)
    const totalPagesNeeded = processes.reduce((sum, procSize) => {
      const breakdown = PagingSegmentSimulator.breakdownSize(procSize);
      const segmentPages = Object.values(breakdown)
        .map((segSize) => Math.ceil(segSize / pageSize))
        .reduce((a, b) => a + b, 0);
      return sum + segmentPages;
    }, 0);
    ...
    // Calculate total pages needed for all processes
    const totalPagesNeeded = processes.reduce((sum, procSize) => {
      return sum + Math.ceil(procSize / pageSize);
    }, 0);
  ...
  document
    .querySelectorAll("#randomize-value")
    .forEach((btn) => (btn.disabled = true));
  document
    .querySelectorAll("#add-process-btn")
    .forEach((btn) => (btn.disabled = true));
  document
...
const runStep = () => {
  ...
  if (isSegmentationPaging) {
    ...
    console.log(
      `SegmentationPaging Step: Index=${processIndex}, SubStep=${simulationState.subStep}`,
    );
    ...
    if (simulationState.subStep === 0) {
      ...
      const remainingFreeFrames = framesArray.filter(
        (f) => f.status === "Free",
      ).length;
      ...
      if (processAllocatable) {
        Object.entries({
          ...
          const { pages, internalFragmentation } =
            PagingSegmentSimulator.segmentToPages(
              segmentSize,
        ...
        appendConsoleMessage(
          `${processName} (${processSize} KB) -> Logical segments allocated.`,
        );
      ...
      // Step 2: Physical Allocation (Frames) - Now one by one
      console.log(
        `Performing Physical Allocation for ${processName} (Page ${simulationState.pageAllocationIndex})`,
      );
      const currentProcess = simulationState.processResults[processIndex];

      if (currentProcess && currentProcess.segments) {
        // Flatten all pages from all segments into a single list
        const allPages = [];
        ...
        if (simulationState.pageAllocationIndex < allPages.length) {
          const { segmentType, page } =
            allPages[simulationState.pageAllocationIndex];
          const allocationResult =
            PagingSegmentSimulator.allocatePageStepSingle(
              simulationState.memory.frames,
        ...
        // If all pages for this process are now allocated
        if (simulationState.pageAllocationIndex >= allPages.length) {
          appendConsoleMessage(
        ...
        console.error(
          "Could not find current process results for physical allocation!",
        );
        simulationState.currentIndex += 1;
        simulationState.subStep = 0;
      }
    ...
    const summary =
      PagingSegmentSimulator.getSimulationSummary(simulationState);
    if (summary) {
  ...
  if (isSegmentation) {
    ...
    const stepResult = allocateNextProcess();
    if (!stepResult) return false;

    // Build a human-readable console message for this individual segment
    const segLabel = stepResult.segmentType
      ? `${stepResult.segmentType.charAt(0).toUpperCase() + stepResult.segmentType.slice(1)} segment`
      : "Segment";
    appendConsoleMessage(
  ...
  if (isPaging) {
    ...
    // Increment total pages allocated
    simulationState.stats.totalPagesAllocated += 1;
    ...
    // Success rate is now based on pages allocated vs total pages needed
    const successRate =
...
const getActiveView = () => {
  ...
  if (standardView && standardView.style.display !== "none")
    ...
    activeView = pagingView;
  else if (segmentationView && segmentationView.style.display !== "none")
    activeView = segmentationView;
  else if (segPagingView && segPagingView.style.display !== "none")
    activeView = segPagingView;
  ...
  if (
    ...
    switch (algoWhat) {
      case "Paging":
        ...
        break;
      case "Segmentation":
        window.location.href = `algorithm/index.html?algorithm=segmentation`;
        break;
      case "Seg-Paging":
        window.location.href = `algorithm/index.html?algorithm=seg-paging`;
        break;
    }
  }
}
...
function simulatorLoad() {
  ...
  const selectedPartition =
    urlPartition || sessionStorage.getItem("selectedPartition");
  ...
  if (selectedAlgo) {
    ...
    if (
      selectedPartition &&
      selectedAlgo.toLowerCase() !== "paging" &&
      selectedAlgo.toLowerCase() !== "segmentation" &&
      selectedAlgo.toLowerCase() !== "seg-paging"
    ) {
